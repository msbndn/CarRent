...
/* g++ database.cpp database.h carrent.cpp -o db */
...
int main() {
    while(true) {
        if(database::getLoggedUsername().empty()) {
            ...
            std::ifstream datafile(usersPath + database::getLoggedUsername() + ".txt");
            if(!datafile.is_open()) {
                database::logOut();
                datafile.close();
                continue;
...
void help() {
    ...
    std::ifstream helpfile(helpPath + "carrentHelp.txt");
    if(helpfile.is_open()){
        std::string line;
